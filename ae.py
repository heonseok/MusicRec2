...
class AE():
    ...
    def build_model(self):
        with tf.device('/gpu:%d' % self.gpu_id):
            ...
            #self.total_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.X, labels=tf.nn.sigmoid(self.recon_X)))
            #self.total_loss = tf.losses.mean_squared_error(self.X, self.recon_X_prob)
            self.total_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.recon_X_logit, labels=self.X))
